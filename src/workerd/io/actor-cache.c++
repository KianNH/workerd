...
kj::Maybe<kj::Promise<void>> ActorCache::getBackpressure() {
  if (dirtyList.size() > lru.options.dirtyListByteLimit && !lru.options.neverFlush) {
  ...
  // it's by default 2MB of data). So if the cache is over the soft limit (which is typically more
